...
```
-df.head(): Untuk melihat mengenai type data dari masing masing kolom kita bisa menggunakan property info.
```
```
-df.info(): Menampilkan informasi tentang DataFrame, termasuk tipe data dan apakah ada nilai yang hilang.
```
```
-sns.heatmap(df.isnull()): Menggunakan Seaborn untuk membuat peta panas yang menunjukkan nilai-nilai yang hilang dalam data.
```
```
-df.describe().T: Menampilkan statistik deskriptif untuk setiap kolom dalam DataFrame.
